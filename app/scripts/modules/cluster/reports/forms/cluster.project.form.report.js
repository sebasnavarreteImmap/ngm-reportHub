				...
				copyPrevReport: function(){
					...
					ngmData.get(getPrevReport).then(function (prev_report) {
						...
						if(!brows && !trows){
							...
							Materialize.toast($filter('translate')('copying'), 1500, 'note');
							if ( !brows && trows > 0 ){
									var msg = $filter('translate')('copied_training')+' ' + trows + ' ' +$filter('translate')('rows');
									typ = 'success';
							} else if ( brows > 0 && !trows ){
								var msg = $filter('translate')('copied_beneficiaries')+' '+ brows + ' '+$filter('translate')('rows');
									  typ = 'success';
							} else{
									var msg = $filter('translate')('copied_beneficiaries')+' ' + brows + ' '+$filter('translate')('rows')+ ' '+$filter('translate')('and')+' ' + $filter('translate')('trainings')+' ' + trows + ' '+$filter('translate')('rows');
										  typ = 'success';
							}
                ...
                ngmClusterBeneficiaries.setLocationsForm( $scope.project.lists, $scope.project.report.locations );
							...
							$timeout(function () {
								...
								if(countNewLocation>0){
									msg += ' '+$filter('translate')('and')+' '+countNewLocation+' '+$filter('transalte')('location')
								}
						...
						Materialize.toast($filter('translate')('error_no_copiado'), 3000, 'error');
						$scope.addBeneficiaryDisable = false;
						$scope.deactivedCopybutton = false;
					})
				},
				...
				uploadDocument: {
					...
					init: function () {
						...
						myDropzone.on("maxfilesexceeded", function (file) {
							...
							document.querySelector(".dz-default.dz-message").style.display = 'block'
							Materialize.toast($filter('translate')('too_many_file_to_upload'),3000,"error")
							$("#upload_doc").attr("disabled", true);
							document.getElementById("upload_doc").style.pointerEvents = "none";
							$("#delete_doc").attr("disabled", true);
							document.getElementById("delete_doc").style.pointerEvents = "none";
